    ...
    <div className="min-h-screen void-bg px-4 py-8">
                ...
                <CardHeader className="flex flex-row items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-muted">
                      {slot.isEmpty ? <Save className="h-6 w-6 text-muted-foreground" /> : <Play className="h-6 w-6 text-primary" />}
                    </div>
                    <div>
                      <CardTitle className="font-title">{slot.isEmpty ? `Slot ${slot.id}` : slot.username}</CardTitle>
                  ...
                  {!slot.isEmpty && (
                    <Button variant="ghost" size="icon" className="opacity-0 group-hover:opacity-100 transition-opacity"
                      onClick={(e) => { e.stopPropagation(); setDeleteModal(slot.id); setDeleteInput(""); }}>
                ...
                {!slot.isEmpty && (
                  <CardContent>
                    <div className="flex gap-4 text-sm font-body text-muted-foreground">
                      {slot.inTreeInstance && <span className="text-accent">Currently Delving</span>}
      ...
      <Dialog open={createModal !== null} onOpenChange={() => setCreateModal(null)}>
        <DialogContent>
          ...
          <Input placeholder="Summoner name..." value={usernameInput} onChange={(e) => setUsernameInput(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && handleCreateSubmit()} className="font-body" />
          <DialogFooter>
      ...
      <Dialog open={deleteModal !== null} onOpenChange={() => setDeleteModal(null)}>
        <DialogContent>
          ...
          <Input placeholder="Type username to confirm..." value={deleteInput} onChange={(e) => setDeleteInput(e.target.value)} className="font-body" />
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteModal(null)}>Cancel</Button>
            <Button variant="destructive" onClick={handleDeleteSubmit}
              disabled={deleteInput !== saveSlots.find(s => s.id === deleteModal)?.username}>Delete Forever</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
