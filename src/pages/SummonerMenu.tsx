...
const SummonerMenu = () => {
  ...
  const handleDelve = () => {
    navigate(slot.inTreeInstance ? "/tree-instance" : "/tree-select");
  };
    ...
    <div className="min-h-screen void-bg px-4 py-8">
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8 text-center">
          <Button size="lg" className="h-14 px-10 text-lg font-title tracking-wide fantasy-border hover:card-glow transition-shadow duration-300" onClick={handleDelve}>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {CHARACTERS.map((char) => {
            ...
            return (
              <Card key={char.id} className="group cursor-pointer fantasy-border transition-all duration-300 hover:card-glow hover:scale-[1.02]"
            ...
            <>
              ...
              <div className="flex justify-center gap-4 mb-2">
                <div className="flex items-center gap-1 bg-muted/50 px-3 py-1 rounded">
                  <Zap className="h-4 w-4 text-energy" /><span className="font-title text-sm">{selectedStats.energy} Energy</span>
                </div>
                <div className="flex items-center gap-1 bg-muted/50 px-3 py-1 rounded">
                  <ChevronUp className="h-4 w-4 text-primary" /><span className="font-title text-sm">{selectedStats.cardDraw} Draw</span>
                </div>
                <div className="flex items-center gap-1 bg-muted/50 px-3 py-1 rounded">
                  <Shield className="h-4 w-4 text-armor" /><span className="font-title text-sm">{selectedStats.armor}% Armor</span>
                </div>
              </div>
              <p className="text-sm text-muted-foreground font-body text-center mb-4">{selectedChar.description}</p>
              <div className="flex justify-center gap-2 mb-4">
                <Button variant={showDeck ? "outline" : "default"} size="sm" onClick={() => setShowDeck(false)}>Stats</Button>
                <Button variant={showDeck ? "default" : "outline"} size="sm" onClick={() => setShowDeck(true)}>Deck ({selectedDeck.length})</Button>
              </div>
              ...
              {!showDeck ? (
                    ...
                    return (
                      <div key={key} className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <Button size="sm" variant="outline" className="h-7 text-xs" disabled={!canAfford}
                            onClick={(e) => { e.stopPropagation(); upgradeStat(selectedChampion!, key); }}>
                            <ChevronUp className="h-3 w-3 mr-1" />{cost}<Coins className="h-3 w-3 ml-1 text-gold" />
                          </Button>
                        </div>
                      </div>
                    );
                ...
                <div className="grid grid-cols-3 gap-2">
                  {selectedDeck.map((card) => (
                    <div key={card.id} className={`rounded-lg border p-2 text-center text-xs ${
                      card.type === "attack" ? "border-destructive/50 bg-destructive/10" :
                      card.type === "skill" ? "border-defense/50 bg-defense/10" :
